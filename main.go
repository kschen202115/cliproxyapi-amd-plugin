...
func apiKeyPage(state string) []byte {
	...
	return []byte(`<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AMD Radeon Cloud</title><style>body{font:15px system-ui,sans-serif;max-width:720px;margin:40px auto;padding:0 20px;color:#18212f}h1{margin-bottom:4px}h2{margin-top:32px}label{display:block;font-weight:600;margin-top:16px}input{box-sizing:border-box;width:100%;margin-top:6px;padding:10px;border:1px solid #b8c2cf;border-radius:7px}button{margin:14px 8px 0 0;padding:9px 14px;border:0;border-radius:7px;background:#d64b2a;color:white;font-weight:700;cursor:pointer}button.secondary{background:#526273}.hint{color:#526273;font-size:13px}.notice{margin:16px 0;white-space:pre-wrap}.card{border:1px solid #d7dee7;border-radius:9px;margin-top:12px;padding:14px}.card h3{margin:0 0 6px}.quota{margin-top:10px;background:#f4f6f8;border-radius:6px;padding:10px;white-space:pre-wrap;font:13px ui-monospace,SFMono-Regular,Menlo,monospace}.error{color:#b42318}</style><main><h1>AMD Radeon Cloud</h1><p class="hint">添加 Radeon API Key，或手动刷新任意 AMD 凭证的额度。刷新会读取 Radeon 当前的每日限额、余额、RPM 和当日用量。</p><details open><summary><strong>管理密钥</strong>（不会存入浏览器）</summary><label>CLIProxyAPI Management Key<input id="managementKey" type="password" autocomplete="off" placeholder="仅当前页面使用"></label><button class="secondary" id="load" type="button">加载 AMD 凭证</button></details><section id="credentials"><h2>额度</h2><p class="hint">点击“加载 AMD 凭证”，然后点击对应凭证的“刷新额度”。</p></section><h2>添加 API Key</h2><form id="form"><label>Radeon API Key<input id="key" type="password" autocomplete="off" placeholder="rc-..." required></label><label>名称（可选）<input id="label" maxlength="80" placeholder="AMD Radeon Cloud"></label><button>保存 API Key</button></form><p id="result" class="notice" role="status"></p></main><script>const state=` + string(stateJSON) + `;const result=document.getElementById('result');const box=document.getElementById('credentials');function storedKey(){try{const direct=['managementKey','management-key','management_password','managementPassword'];for(const k of direct){const v=localStorage.getItem(k);if(v)return v}for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i)||'';if(/management.*(key|password)|(key|password).*management/i.test(k)){const v=localStorage.getItem(k);if(v)return v}}}catch(_){}return ''}function managementKey(){return document.getElementById('managementKey').value.trim()||storedKey()}function headers(){const key=managementKey();if(!key)throw new Error('请先填写 CLIProxyAPI 管理密钥。');return {'Content-Type':'application/json','Authorization':'Bearer '+key}}async function readJSON(response){const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.message||data.error||('请求失败：HTTP '+response.status));return data}function line(metric){const v=Number(metric.value||0);if(metric.format==='currency')return metric.label+': '+new Intl.NumberFormat('en-US',{style:'currency',currency:metric.currency||'USD',maximumFractionDigits:4}).format(v);return metric.label+': '+new Intl.NumberFormat().format(v)+(metric.unit?' '+metric.unit:'')}function renderQuota(target,data){target.textContent=(data.summary||[]).map(line).join('\n')||'Radeon 没有返回额度数据。';const bucket=data.groups&&data.groups[0]&&data.groups[0].buckets&&data.groups[0].buckets[0];if(bucket)target.textContent+='\n\n每日余额：'+Math.round(Number(bucket.remainingFraction||0)*100)+'%\n重置时间：'+(bucket.resetTime||'未知')}function credentialCard(item){const card=document.createElement('article');card.className='card';const title=document.createElement('h3');title.textContent=item.name||item.id||'AMD 凭证';const meta=document.createElement('div');meta.className='hint';meta.textContent='认证索引：'+(item.auth_index||'未知');const button=document.createElement('button');button.type='button';button.textContent='刷新额度';const quota=document.createElement('div');quota.className='quota';quota.hidden=true;button.onclick=async()=>{try{button.disabled=true;button.textContent='刷新中…';const r=await fetch('/v0/management/plugins/amd/quota',{method:'POST',headers:headers(),body:JSON.stringify({auth_index:item.auth_index})});renderQuota(quota,await readJSON(r));quota.hidden=false}catch(err){quota.textContent='刷新失败：'+err.message;quota.className='quota error';quota.hidden=false}finally{button.disabled=false;button.textContent='刷新额度'}};card.append(title,meta,button,quota);return card}async function loadCredentials(){try{box.innerHTML='<h2>额度</h2><p class="hint">加载中…</p>';const r=await fetch('/v0/management/auth-files',{headers:headers()});const data=await readJSON(r);const files=(data.files||[]).filter(x=>String(x.provider||x.type||'').toLowerCase()==='amd');box.innerHTML='<h2>额度</h2>';if(!files.length){box.insertAdjacentHTML('beforeend','<p class="hint">还没有 AMD 凭证。请在下方添加 API Key。</p>');return}files.forEach(x=>box.appendChild(credentialCard(x)))}catch(err){box.innerHTML='<h2>额度</h2><p class="error">'+err.message+'</p>'}}document.getElementById('load').onclick=loadCredentials;document.getElementById('form').addEventListener('submit',async e=>{e.preventDefault();result.textContent='保存中…';try{const key=document.getElementById('key').value.trim();const label=document.getElementById('label').value.trim();const r=await fetch('/v0/management/plugins/amd/credentials',{method:'POST',headers:headers(),body:JSON.stringify({api_key:key,label,state})});await readJSON(r);document.getElementById('key').value='';result.textContent='已保存。认证窗口会自动完成；现在可点击“加载 AMD 凭证”后刷新额度。';await loadCredentials()}catch(err){result.textContent='保存失败：'+err.message}});</script></html>`)
}
